...
Draupnir Core 1.8.1 is primarily a bugfix release, bringing Draupnir Core in
line with Bitcoin 0.9.3. Draupnir Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Draupnir Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Draupnir Core clients
* Use HTTPS with draupnirchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/draupnir/draupnir/issues/134).
...
This only affects values significantly below 1 Draupnir RING, however merchants may wish to round to
the nearest RING on all transactions for now.
