// Copyright (c) 2021 The Draupnir Core developers
...
// Dogiecoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateDraupnirNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetDraupnirMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetDraupnirDustFee(tx.vout, ::minRelayTxFee);
