...
#if defined(NDEBUG)
# error "Draupnir cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Draupnir Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetDraupnirMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Draupnir: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("draupnir-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Draupnir: BIP30 has been active since inception
    ...
    // Draupnir: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetDraupnirBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Draupnir: Disallow AuxPow blocks before it is activated.
    ...
    // Draupnir: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Draupnir: We probably want to disable this
    int nLockTimeFlags = 0;
