...
const QString BITCOIN_IPC_PREFIX("draupnir:");
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (QFile::exists(arg)) // Filename
        {
            ...
            if (readPaymentRequestFromFile(arg, request))
            {
                ...
                if (request.getDetails().genesis() == "fbb770b8bc37dac0fc8cfc27542a0fb38cea5deddd493ea8df830c6ef0d3f90d") // SKOL
                {
                    SelectParams(CBaseChainParams::MAIN);
                }
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start draupnir: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Draupnir address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
